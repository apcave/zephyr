...
#define VL53L4CD_ERROR_CONTROL_INTERFACE	((uint8_t)252U)
...
#define VL53L4CD_IDENTIFICATION__MODULE_TYPE    ((uint16_t)0x0110)
...
/**
 * @brief This function detects if a VL53L4CD sensor is present on the I2C bus.
 * It can be used at the beginning of the application to check if the sensor is
 * correctly connected and powered on.
 * @param (Dev_t) dev : pointer of device instance to initialize if sensor is
 * detected.
 * @return (VL53L4CD_ERROR) status : 0 if update is OK.
 */

VL53L4CD_Error VL53L4CD_detect(Dev_t dev);

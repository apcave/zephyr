...
LOG_MODULE_REGISTER(api, CONFIG_VL53L4CD_LOG_LEVEL);
...
#ifdef CONFIG_VL53L4CD_I2C_DETECT
static void i2c_detect(const struct i2c_dt_spec *i2c)
{
	char output[80*18];
	int len = 0;
	output[0] = '\0';

	len += snprintf(output + len, sizeof(output) - len, "Scanning I2C bus %s for devices...\n", i2c->bus->name);
	len += snprintf(output + len, sizeof(output) - len, "    00 01 02 03 04 05 06 07 08 09 0a 0b 0c 0d 0e 0f\n");
	for (int i = 0; i < 16; i++) {
		len += snprintf(output + len, sizeof(output) - len, "%02x:", i*16);
		for (int j = 0; j < 16; j++) {
			uint8_t addr = i*16 + j;
			uint8_t dummy = 0;
			struct i2c_msg msg = {
			...
			int ret = i2c_transfer(i2c->bus, &msg, 1, addr);
			if (ret == 0) {
				len += snprintf(output + len, sizeof(output) - len, " %02x", addr);
			} else {
...
VL53L4CD_Error VL53L4CD_detect(
	Dev_t dev)
{
	VL53L4CD_Error status = VL53L4CD_ERROR_NONE;
	...
	// The first read often fails.
	i2c_write_read_dt(dev->i2c, (uint8_t*)&reg_null, 2, (uint8_t*)&reg_null, 1);

#ifdef CONFIG_VL53L4CD_I2C_DETECT
	i2c_detect(dev->i2c);
#endif

	status |= VL53L4CD_RdByte(dev, VL53L4CD_IDENTIFICATION__MODEL_ID, &module_id);
	if (status) {
		return VL53L4CD_ERROR_CONTROL_INTERFACE;
	}

	status |= VL53L4CD_RdByte(dev, VL53L4CD_IDENTIFICATION__MODULE_TYPE, &module_type);
	if (status) {
		return VL53L4CD_ERROR_CONTROL_INTERFACE;
	}

	if ((module_id != 0xEB) || (module_type != 0xAA)) {
		LOG_ERR("Error to read module id 0x%x, module type 0x%x", module_id, module_type);
		return VL53L4CD_ERROR_CONTROL_INTERFACE;
	}
	LOG_INF("stmVL53L4CD: module_id : 0x%x. module_type : 0x%x", module_id, module_type);
